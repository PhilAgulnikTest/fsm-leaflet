/* CLI: import the entitledto LA client list from the existing sites CSV.
 *
 * Usage:
 ...
 *
 * Mapping:
 *   - slug derived from the URL subdomain when present, else from a kebab-cased
 *     name (with "calc"/"council"/"borough" trimmed for cleanliness)
 *   - calculator_subdomain: extracted from the URL, or set to the generic
 *     "<slug>.entitledto.co.uk" placeholder if the row had no URL
 *   - default branding: entitledto house colours (override via the admin UI later)
 *   - notes: flag whether the row had a real URL or fell back to the placeholder
...
const DEFAULT_BRAND = '#1F4F5C';   // entitledto dark teal
const DEFAULT_ACCENT = '#2563EB';  // strong blue — placeholder until LA brand sourced
...
// The CSV has a 2-line title block before the real header. Find the header row
// dynamically rather than hard-coding the offset.
const lines = csv.split(/\r?\n/);
const headerIdx = lines.findIndex((l) => /^\s*#\s*,/.test(l));
if (headerIdx < 0) {
  console.error('Could not locate header row (looking for "#,Local Authority,...").');
  process.exit(1);
}
...
const tx = db.transaction(() => {
  for (const r of rows) {
    ...
    let slug = (url && slugFromUrl(url)) || slugFromName(name);
...
// Direct-run guard (matches the pattern used in migrate.ts).
const thisFile = fileURLToPath(import.meta.url);
const entry = process.argv[1] ? path.resolve(process.argv[1]) : '';
void thisFile; void entry; // unused — execution is at module top-level here
